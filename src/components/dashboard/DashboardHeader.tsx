...
import type { User, Hospital } from '@/types/database.types';
...
export default function DashboardHeader({ user, hospital }: DashboardHeaderProps) {
          ...
          <div className="relative">
            ...
            >
              <div className="bg-white text-purple-600 rounded-full h-8 w-8 flex items-center justify-center shadow-sm">
                <span className="text-sm font-semibold">{hospital?.name?.charAt(0) || user?.organizationName?.charAt(0) || 'H'}</span>
              </div>
              <span className="hidden md:inline-block font-medium text-white">
                {hospital?.name || user?.organizationName || 'Hospital Admin'}
              </span>
